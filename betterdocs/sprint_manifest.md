...
| CLI Feature | `module find` command | `cmd/jpm/find.go` | Accepts optional path, resolves absolute roots, prints indexed module list. |
...
|----------|-----------|-----------|--------------|
| P0 | `jpm deps show` (Maven) | Provide immediate value via read-only dependency visibility | Stable POM parsing, CLI command scaffolding. |
